...
function GraphicCards({ items, styles }) {
  return (
    <>
...
export default function MainGraphicAccordion() {
  ...
  const graphicSolutionsData = useMemo(() => {
    const metaMap = new Map(graphicSolutionsMeta.map((item) => [item.id, item]));
  ...
  const activeIndex = useMemo(() => {
    const index = graphicSolutionsData.findIndex((item) => item.id === activeTab);
    return index === -1 ? 0 : index;
  }, [activeTab, graphicSolutionsData]);

  const activeSolution = useMemo(() => {
    return graphicSolutionsData[activeIndex] || null;
  }, [activeIndex, graphicSolutionsData]);
      ...
      <div key={activeSolution.id} className={styles.allDetails}>
        {activeSolution.sections.map((section, index) => (
          <div className={styles.sec} key={index}>
            ...
            {section.type === "cards" && (
              <GraphicCards items={section.items} styles={styles} />
            )}
          </div>
        ))}
